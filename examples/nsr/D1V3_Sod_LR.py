...
@hydra.main(version_base='1.2', config_path="config", config_name="Sod_D1V3_LR")
def train(config):
    print(config)
    nv=config["vmesh"]["nv"]
    ...
    #model=SplitNet3D(config.network.neurons , config.network.rank, VT, config.network.multires)
    ...
    for i in range(config.optim["Adam_steps"]):
        ...
        # print(IV_pred[0].shape,IV_pred[1].shape,IV_pred[2].shape)
        # print(IV_diff[0].shape,IV_diff[1].shape,IV_diff[2].shape)
